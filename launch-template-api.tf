## API LAUNCH TEMPLATE ##
resource "aws_launch_template" "ecs-cicd-launch-template-api" {
  name          = "ecs-launch-template-api"
...
echo ECS_CLUSTER=${aws_ecs_cluster.ecs-cluster-api.name} >> /etc/ecs/ecs.config
  EOF
  )
